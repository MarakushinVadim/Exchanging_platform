...
class AdTestCase(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            username="test_user", email="test_user@mail.ru", password="testPassword21"
        )
...
class ExchangeProposalTestCase(TestCase):
    def setUp(self):
        ...
        self.user_1 = User.objects.create_user(
            ...
            email="test_1_user@mail.ru",
            password="testPassword21",
        )
        self.user_2 = User.objects.create_user(
            username="test_2_user",
            email="test_2_user@mail.ru",
    ...
    def test_create_view(self):
        ...
        response = self.client.post(
            reverse("ads:exchange_proposal_create", args=[self.obj_2.pk]), data
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_accept_proposal(self):
        ...
        response = self.client.post(
            reverse("ads:accept_proposal", args=[self.object.pk])
        )
    ...
    def test_reject_proposal(self):
        self.client.login(username="test_2_user", password="testPassword22")
        response = self.client.post(
            reverse("ads:reject_proposal", args=[self.object.pk])
        )
        self.object.refresh_from_db()
        self.assertEqual(response.status_code, 302)
